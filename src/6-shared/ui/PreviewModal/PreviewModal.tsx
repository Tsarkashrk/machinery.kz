...
export const PreviewModal = <T extends Record<string, any>>({
  isOpen,
  onClose,
  item,
  title,
    ...
    switch (field.type) {
      ...
      case "images":
        if (!Array.isArray(value) || value.length === 0) {
          return (
            <span className="preview-empty">Изображения не загружены</span>
          );
        }
        return (
            ...
            {value.map((imageObj, index) => (
              <div key={imageObj.id || index} className="preview-image-item">
                <img
                  ...
                  className="preview-image__img preview-image__small"
                />
                <div className="preview-image-info">
                  <span className="preview-image-type">
                    {imageObj.image_type}
                  </span>
                  <span className="preview-image-date">
                    {new Date(imageObj.uploaded_at).toLocaleDateString("ru-RU")}
                  </span>
                </div>
              </div>
            ))}
  ...
  const renderField = (field: FieldConfig) => {
    ...
    return (
      <div key={field.name} className="preview-field">
        ...
        <div className="preview-field__value">
