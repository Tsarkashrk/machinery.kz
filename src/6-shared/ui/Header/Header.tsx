...
const Header = ({ isSimple }: Props) => {
  ...
  const actionLinks = [
    {
      link: `${PLATFORM_PAGES.MESSAGES}`,
      title: tNav('messages-title'),
      icon: <MessagesSquare size={ICON_SIZE} />,
    },
    {
      link: `${PLATFORM_PAGES.FAVORITES}`,
      title: tNav('favorites-title'),
                ...
                <div className="header__actions">
                  {actionLinks.map((item) => (
                    <Link
                      key={item.link}
                      href={item.link}
                      className={`header__link ${path === item.link && 'header__link--active'}`}
                    >
                      {item.icon}
                    </Link>
                  ))}
