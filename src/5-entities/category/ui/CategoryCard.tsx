  ...
  return (
    <Link href={`${PLATFORM_PAGES.PRODUCTS}/?category=${category.id}`} className={`category-card ${(category.id === 7 || category.id === 20) && 'category-card--doubled'} ${category.id >= 27 && 'category-card--machinery category-card--doubled'}`}>
      ...
      <div className="category-card__image-container">
        <Image src={category.file ? category.file : `/assets/cat${category.id}-d.${category.id >= 31 ? 'jpeg' : category.id >= 27 ? 'png' : 'webp'}`} alt={category.name} className="category-card__image" width={400} height={400} />
      </div>
    </Link>
  )
}
