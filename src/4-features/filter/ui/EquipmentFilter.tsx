...
import { convertFormDataToFilters, getInitialFormData } from '../lib/filter.utils'
...
export const EquipmentFilter: React.FC<IEquipmentFilterProps> = ({ onFiltersChange, initialFilters = {}, className = '' }) => {
  const [isOpen, setIsOpen] = useState(false)
  const [showAllFilters, setShowAllFilters] = useState(false)

  const { brands, isLoading: brandsLoading } = useBrands()
  const { categories, isLoading: categoriesLoading } = useCategories({ ordering: 'name' })

  const defaultValues = getInitialFormData()
  const { register, handleSubmit, reset, watch, control } = useForm<IFilterFormData>({
    defaultValues,
  })
  ...
  const filterFields = [
    {
      label: 'Поиск',
      element: <Input placeholder="Поиск по названию..." {...register('search')} />,
    },
    {
      label: 'Категория',
      element: <Dropdown control={control} name={'category'} options={categories?.map((c) => ({ ...c, title: c.name, value: c.id })) || []} />,
    },
    {
      label: 'Бренд',
      element: <Dropdown control={control} name={'brand'} options={brands?.map((b) => ({ ...b, title: b.name, value: b.id })) || []} />,
    },

    {
      label: 'Цена от',
      element: <Input type="number" placeholder="0" {...register('min_price')} />,
    },
    {
      label: 'Аренда в день от',
      element: <Input type="number" placeholder="0" {...register('min_rental_rate')} />,
    },
    {
      label: 'Год от',
      element: <Input type="number" placeholder="1900" {...register('min_year')} />,
    },
    {
      label: 'Цена до',
      element: <Input type="number" placeholder="∞" {...register('max_price')} />,
    },
    {
      label: 'Аренда в день до',
      element: <Input type="number" placeholder="∞" {...register('max_rental_rate')} />,
    },
    {
      label: 'Год до',
      element: <Input type="number" placeholder={new Date().getFullYear().toString()} {...register('max_year')} />,
    },
    {
      label: 'Доступно для аренды',
      element: (
        <div style={{ display: 'flex', gap: '1rem' }}>
          <input type="checkbox" {...register('available_for_rent')} /> <Description>Доступно для аренды</Description>
        </div>
      ),
    },
    {
      ...
      <div className="equipment-filter__wrapper">
        <form className="equipment-filter__panel" onSubmit={handleApplyFilters}>
          <div className="equipment-filter__grid">
            {(showAllFilters ? filterFields : filterFields.slice(0, 3)).map((field, index) => (
