...
export const UpdateUserModal = ({ isOpen, onClose, user, onSave, isLoading = false }: UpdateUserModalProps) => {
  const {
    register,
    handleSubmit,
    control,
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} size="lg" closeOnOverlayClick={!isLoading}>
      ...
      <ModalContent>
        <form onSubmit={handleSubmit(onSubmit)} className="modal-form" id="user-form">
      ...
      <ModalFooter>
        ...
        <button type="submit" form="user-form" disabled={isLoading} className="modal-btn modal-btn--primary">
